...
A popular misunderstanding of this number is "Rust would have prevented 70% of these vulnerabilities _and no other systems language would have prevented any of them_." This misunderstanding can be corrected by reading the presentation's next slide (Slide 11), which has a breakdown by type of memory safety issue. It paints a completely different picture when it comes to, say, Rust and Zig:
...
- 16.4% of the vulnerabilities were specifically use-after-free errors. In these cases, the objectively safest choice would be Zig with ReleaseSafe, followed by Rust with no `unsafe` anywhere, followed by either Zig with ReleaseFast or Rust, depending on how much `unsafe` the Rust had in its dependency tree.

Zig's ReleaseSafe handles use-after-free errors in the safest way: with runtime bookkeeping that quite literally checks whether you are about to use freed memory, and if so, panics instead of permitting memory unsafety. This is as safe as Safe Rust (although slower at runtime, and less pleasant because of the panic), and it is _astronomically safer_ than Unsafe Rust—which is found (usually in small quantities) all over the Rust ecosystem, including in Rust's standard library.

Approximately every production Rust program has some amount of Unsafe Rust somewhere in its dependencies, which is what makes Zig's ReleaseSafe objectively the safest choice if you are trying to maximize memory safety. The [TigerBeetle](https://tigerbeetle.com/) financial database uses Zig with ReleaseSafe, and recently underwent a legendarily meticulous [Jepsen report that found only two safety bugs](https://tigerbeetle.com/blog/2025-06-06-fuzzer-blind-spots-meet-jepsen/), neither related to memory safety. If TigerBeetle rewrote in Rust, they would become less memory-safe.

Of course, the reason Rust's borrow checker is such a big deal is that it can detect use-after-frees at compile time _without_ the sort of runtime checks and panics that tools like Zig's ReleaseSafe use. Nobody wants runtime panics, and most projects that choose a systems language do so because they want to maximize performance. Both of these are compelling reasons to choose Rust! It's perfectly reasonable to say "I am choosing Rust instead of Zig's ReleaseSafe becuase I want my program to run faster and have a lower chance of panicking, even if it is less memory-safe than it would be with Zig." Creating software people love is about thoughtful tradeoffs, not thoughtless absolutes.

Zig's [ReleaseFast](https://ziglang.org/documentation/master/#toc-ReleaseFast) is where things get really interesting. It skips the runtime use-after-free checks in production, while keeping them in debug builds and tests, to help catch memory-safety bugs during development. This is not as safe as Safe Rust, but it's still safer than Unsafe Rust, which has all the unsafety and none of the assistence in detecting it during development. So the more `unsafe` you have in your Rust code base, the more likely it is that Zig's ReleaseFast might have helped you avoid more memory-unsafety issues in practice.

How has this shaken out in practice? The widely-used [Ghostty](https://ghostty.org/) terminal emulator uses Zig with ReleaseFast, and it has [never had a memory-safety CVE](https://app.opencve.io/cve/?vendor=ghostty). Zig's compiler is in the same boat: ReleaseFast builds, [no memory-safety CVEs in practice](/todo/link/to/this). Deno, a Bun competitor which is written in Rust (and, if I had to guess, used more widely than Zig but less widely than Ghostty), has had memory-unsafety CVEs. They include an [out-of-bounds read](https://osv.dev/vulnerability/GHSA-c25x-cm9x-qqgx) as well as a [use-after-free](https://osv.dev/vulnerability/GHSA-3j27-563v-28wf), both involving the use of Unsafe Rust. (For what it's worth, I personally doubt Zig's ReleaseSafe would have caught either of these specific issues either.)

Bun has written about their struggles to achieve the track records of TigerBeetle, Ghostty, and Zig, and their logic seems reasonable to me for expecting that Rust's toolset (borrow-checking plus `Drop` in particular) could get them to a better outcome. But Rust can't guarantee freedom from memory-unsafety bugs in practice for Bun, just like it hasn't for Deno. It can offer 

I don't know of any studies comparing the efficacy of Rust's borrow checker and Zig's "panic on attempted use-after-frees during development" ReleaseFast strategies in the wild, but at least these two projects seem to suggest you can use ReleaseFast without running into memory-safety issues in practice.

> If you want to learn more about these projects, I've recorded in-depth conversations with their creators: [Joran Greef on TigerBeetle](https://youtu.be/8br5QcmYq84?si=qi7z2z8nSUKxiWlL), [Mitchell Hashimoto on Ghostty](https://youtu.be/ljoNEH39lyw?si=fnm0emKYj5eH3sPP), and [Andrew Kelley on Zig](/todo/link/goes/here).

In Roc, we've followed Ghostty and Zig's lead and chosen Zig with ReleaseFast. How has this worked out for us so far in practice? How many memory corruption incidents have we seen since rewriting our compiler from Rust to Zig?
