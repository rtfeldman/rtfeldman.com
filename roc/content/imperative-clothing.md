...
“Sooner or later you have to do some effects. Otherwise you're just heating up the CPU.”

I've always wanted the purely functional [Roc programming language](https://www.roc-lang.org/) to be delightful for I/O-heavy use cases, including shell scripting. However, when I recently sat down to rewrite a shell script from Bash into Roc, I wasn't happy with how the code ended up feeling.
...
There are some benefits to separating things this way (more on those later), but to illustrate what's happening behind the scenes here, let's go back to that example from earlier:
